...
const OrderInTransit = () => {
  ...
  useEffect(()=>{
    const fetchOrderDetails = async () => {
      try {
        // console.log(filters);
        const response = await orderDetails("In Transit", filters.orderTime, filters.keyword, filters.category,setData);
    ...
    <table className="table-fixed w-full border-collapse">
      ...
      <thead>
        <tr className="bg-white text-black font-normal  shadow-lg rounded-xl">
          ...
          <th className="p-2 md:p-3 text-left font-normal w-40 border-b">EOD</th>
